...
WebChat.prototype = {
	...
	init: function(){
		...
		document.getElementById('changeLayout').addEventListener('click',function(){
			...
			if("wrapperBefore" === wrapper.getAttribute('class')){
				wrapper.setAttribute('class','wrapperChanged');
				historyMsg.setAttribute('class','historyMsgChanged');
				controls.setAttribute('class','controlsChanged');
			}else{
				wrapper.setAttribute('class','wrapperBefore');
				historyMsg.setAttribute('class','historyMsgBefore');
				controls.setAttribute('class','controlsBefore');
			}
		},false);
	},
	...
	showEmoji:function(msg){
		...
		while(match = reg.exec(msg)){
			...
			if(emojiIndex > totalEmojiNum){
				...
				result = result.replace(match[0],'<img class="emoji" src="../content/emoji/'
				 + emojiIndex + '.gif"/>');
			}
		}
		return result;
